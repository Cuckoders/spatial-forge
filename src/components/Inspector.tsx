...
function RoomInspector({ id }: { id: string }) {
  ...
  return <>
    ...
    {room.shape === 'polygon' && room.vertices ? <section className="inspector-section"><div className="inspector-title"><span>Вершины контура</span><Move3D size={16} /></div><p className="vertex-note">Координаты указаны относительно центра объекта. Маркеры в сцене имеют такие же номера.</p><div className="vertex-list">{room.vertices.map((point, index) => <div className="vertex-row" key={index}><b>{index + 1}</b><div className="field-row"><NumericField label="X" max={50} min={-50} onChange={(x) => updatePolygonVertex(room.id, index, { x })} unit="м" value={point[0]} /><NumericField label="Z" max={50} min={-50} onChange={(z) => updatePolygonVertex(room.id, index, { z })} unit="м" value={point[1]} /></div></div>)}</div></section> : null}
