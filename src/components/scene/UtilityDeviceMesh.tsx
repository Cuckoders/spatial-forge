...
import { UTILITY_DEVICE_KINDS, utilityRouteProjection } from '../../lib/utilities';
import { useEditorStore } from '../../store/editorStore';
import type { PlanUtilityDevice, PlanUtilityRoute } from '../../types';
...
export function UtilityDeviceMesh({ device, route, floorElevation, active }: { device: PlanUtilityDevice; route?: PlanUtilityRoute | undefined; floorElevation: number; active: boolean }) {
  ...
  const connection = route ? utilityRouteProjection(route, device.x, device.z) : undefined;
  ...
  return <>
    {route && connection ? <Line color={selected ? '#FFFFFF' : style.color} dashed dashScale={8} dashSize={0.08} gapSize={0.06} lineWidth={selected ? 3 : 2} onClick={onClick}
      opacity={active ? 0.9 : 0.35} points={[[device.x, floorElevation + 0.035, device.z], [connection.x, floorElevation + route.elevation, connection.z]]} transparent /> : null}
    <group onClick={onClick} position={[device.x, y, device.z]} rotation={[0, device.rotation, 0]} scale={active ? 1 : 0.9}>
      {selected || !route ? <mesh position={[0, -y + floorElevation + 0.03, 0]} rotation={[-Math.PI / 2, 0, 0]}><ringGeometry args={[0.34, 0.43, 28]} /><meshBasicMaterial color={selected ? '#FFFFFF' : '#D8583F'} depthTest={false} /></mesh> : null}
      <DeviceSymbol color={selected ? '#FFFFFF' : style.color} device={device} />
      {selected ? <Html center position={[0, 0.54, 0]} style={{ pointerEvents: 'none' }} zIndexRange={[43, 0]}><div className="utility-route-label"><b>{style.shortLabel}</b><span>{route ? route.name : 'Нет связи'}</span></div></Html> : null}
    </group>
  </>;
}
