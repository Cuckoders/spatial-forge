...
import { Calculator, Download, Layers3, Paintbrush, RotateCcw, X } from 'lucide-react';
...
export default function MaterialEstimate({ onClose }: MaterialEstimateProps) {
  ...
  const quantities = useMemo(() => calculateProjectQuantities({ floors, rooms, walls, openings, wallOpenings }), [floors, openings, rooms, wallOpenings, walls]);
  ...
  return <div className="template-catalog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section aria-labelledby="material-estimate-title" aria-modal="true" className="material-estimate" role="dialog">
      <header className="template-catalog-head material-estimate-head">
        <div><span className="eyebrow">Расчёт проекта</span><h2 id="material-estimate-title">Ведомость материалов</h2><p>Площади и ориентировочная стоимость без учёта работ и доставки.</p></div>
